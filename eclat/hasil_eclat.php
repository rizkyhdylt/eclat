...
$query = "SELECT p.bulan, p.tahun, p.status, s.nama_penyewa, s.no_hp, k.tipe_kamar 
          FROM pembayaran p
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($row['status'] == 'Terlambat') {
            ...
            if (!isset($pola_terlambat[$key])) {
                $pola_terlambat[$key] = [
                    'frekuensi' => 0,
                    'no_hp' => $row['no_hp'] // Simpan nomor HP di sini
                ];
            }
          ...
          <tbody>
            ...
            if (count($pola_terlambat) > 0) :
                foreach ($pola_terlambat as $itemset => $data_pola) : 
                    ...
                    // Logika SP & Pesan (Tetap sama seperti sebelumnya)
                    if ($support >= 80) {
                        $status_text = "SP 3 Keluar"; 
                        $badge_class = "action-critical"; 
                        $icon = "fa-handcuffs";
                        $pesan = "⚠️ *SURAT PERINGATAN KETIGA (SP 3) - FINAL*\n\n" .
                                "Yth. *$clean_name*,\n\n" .
                                "Sistem kami mendeteksi tingkat keterlambatan pembayaran Anda sudah mencapai tahap kritikal (" . number_format($support, 1) . "%). Ini adalah peringatan terakhir.\n\n" .
                                "Mohon segera melunasi seluruh tunggakan Anda dalam waktu 1x24 jam. Jika tidak ada konfirmasi, dengan berat hati kami akan memproses pemutusan kontrak dan pengosongan unit secara sepihak. Segera hubungi Admin.";
                    } elseif ($support >= 65) {
                        ...
                        $pesan = "🔸 *SURAT PERINGATAN KEDUA (SP 2)*\n\n" .
                                "Yth. *$clean_name*,\n\n" .
                                "Kami mencatat bahwa Anda telah terlambat melakukan pembayaran sebanyak $frekuensi kali. Hal ini telah melampaui batas toleransi yang disepakati.\n\n" .
                                "Mohon segera melakukan pelunasan agar status kontrak Anda tidak berlanjut ke SP 3 (Pemutusan Kontrak). Harap abaikan pesan ini jika Anda sudah melakukan pembayaran.";
                    } elseif ($support >= 50) {
                        ...
                        $pesan = "🔹 *SURAT PERINGATAN KESATU (SP 1)*\n\n" .
                                "Yth. *$clean_name*,\n\n" .
                                "Melalui pesan otomatis ini, kami informasikan bahwa sistem mendeteksi adanya pola keterlambatan pembayaran pada riwayat kontrak Anda.\n\n" .
                                "Kami mohon kerjasamanya untuk lebih memperhatikan jadwal pembayaran ke depannya agar kenyamanan bersama tetap terjaga. Terima kasih.";
                    } else {
                        $status_text = "Review Bulanan"; 
                        $badge_class = "action-review"; 
            ...
            <tr class="table-row-hover">
                ...
                <td class="text-center">
                    <div class="action-badge <?= $badge_class ?> mb-2">
                        <i class="fa-solid <?= $icon ?>"></i> <span><?= $status_text ?></span>
                    </div>
                    <?php if ($support >= 50) : ?>
                        <div class="d-grid mt-1">
                            <a href="https://api.whatsapp.com/send?phone=<?= $formatted_phone ?>&text=<?= urlencode($pesan) ?>" 
